...
module.exports = {
    ...
    pg.connect(connectionString, function(err, client, endConnection) {
      if(err) {
        ...
        client.query('BEGIN', function(err) {
          if (err) {
            winston.error('BEGIN transaction error', {reason: err});
            releaseErroredConnection(endConnection);
            done(err);
          } else {
            client.rollback = function(callback) {
              client.query('ROLLBACK', function(err) {
                if (err) {
                  winston.error('ROLLBACK transaction error', {reason: err});
                  releaseErroredConnection(endConnection);
                  callback(err);
                } else {
                  endConnection();
            ...
            client.commit = function(callback) {
              client.query('COMMIT', function(err) {
                if (err) {
                  winston.error('COMMIT transaction error', {reason: err});
                  releaseErroredConnection(endConnection);
                  callback(err);
                } else {
                  endConnection();
  ...
  shutdown: function() {
    winston.warn('Calling Postgres end to drain pool.');
    pg.end();
  },
  ...
  transactionMiddleware: function(req, res, next) {
    module.exports.getConnection(function(err, client) {
      if (err) {
        winston.error('Could not retrieve connection for middleware transaction.', {reason: err});
        res.status(500).end();
      } else {
        winston.debug('Populated request object with middleware transaction.');
            ...
            if (req.transaction) {
              if (!process.env.ALWAYS_ROLLBACK && res.statusCode >= 200 && res.statusCode < 300) {
                req.transaction.commit(function(err) {
                  if (err) {
                    winston.error('Failed to commit middleware transaction.', {reason: err});
                    res.status(500).end();
                  } else {
                ...
                req.transaction.rollback(function(err) {
                 if (err) {
                   winston.error('Failed to rollback middleware transaction.', {reason: err});
                   res.status(500).end();
                 } else {
                   winston.warn('Successfully rolled back middleware transaction.');
                   fToOverride.apply(res, [body]);
                 }
               });
              }
