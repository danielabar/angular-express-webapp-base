        ...
        client.query('BEGIN', function(err) {
          if (err) {
            ...
            client.rollback = function(callback) {
              console.log('database client.rollback starting...');
              client.query('ROLLBACK', function(err) {
                if (err) {
                  console.log('database client.rollback err: ' + err);
                  releaseErroredConnection(endConnection);
                  callback(err);
                } else {
                  console.log('database client.rollback success');
                  endConnection();
                  console.log('database client.rollback calling back with null');
                  callback(null);
                }
              });
            };
